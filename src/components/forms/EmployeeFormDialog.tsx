...
export function EmployeeFormDialog({ open, onOpenChange, onSubmit, initialData, loading }: EmployeeFormProps) {
  const [form, setForm] = useState({ name: '', role: '', department: '', start_date: '', leave_balance: 20, email: '', job_title: '' });

  useEffect(() => {
    if (initialData) {
      setForm({ name: initialData.name ?? '', role: initialData.role ?? '', department: initialData.department ?? '', start_date: initialData.start_date ?? '', leave_balance: initialData.leave_balance ?? 20, email: initialData.email ?? '', job_title: initialData.job_title ?? '' });
    } else {
      setForm({ name: '', role: '', department: '', start_date: '', leave_balance: 20, email: '', job_title: '' });
    }
  }, [initialData, open]);
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label>Email</Label><Input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="Optional – links to user account" /></div>
